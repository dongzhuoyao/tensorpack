...
class Model(ModelDesc):
    def inputs(self):
        return [tf.placeholder(tf.float32, [1, support_image_size[0], support_image_size[1], 3], 'first_image_masked'),
                #tf.placeholder(tf.float32, [None, support_image_size[0], support_image_size[1]], 'first_label'),
                tf.placeholder(tf.float32, [1, query_image_size[0], query_image_size[1], 3], 'second_image'),
                tf.placeholder(tf.int32, [1, query_image_size[0], query_image_size[1]], 'second_label')
                ]
    ...
    def build_graph(self, first_image_masked, second_image, second_label):
        ...
        with guided_relu():
            ...
            saliency = saliency_map(logits, second_image, name="saliency")
...
def proceed_saliency(args, is_save = False):
    import cv2
    ds = get_data(args.test_data)


    result_dir = "result_saliency"
    from tensorpack.utils.fs import mkdir_p
    mkdir_p(result_dir)
    ...
    for first_image_masks, second_image, second_label  in tqdm(ds.get_data()):
        ...
        for kk in range(k_shot):
            ...
            saliency_images = predict_scaler(second_image, mypredictor, scales=[0.5,0.75, 1, 1.25, 1.5], classes=3, tile_size=support_image_size, is_densecrf = False)
            ...
            rsl = second_image * 0.2 + abs_saliency * 0.8
        ...
        if is_save:
            cv2.imwrite("{}/{}.png".format(result_dir,i), np.concatenate((second_image,pos_saliency * 255, neg_saliency * 255, abs_saliency,rsl ), axis=1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_data', default="fold3_train", help='train data')
    parser.add_argument('--test', action='store_true', help='test data')
    parser.add_argument('--saliency', action='store_true', help='test data')
    ...
    if args.view:
        ...
        proceed_saliency(args,is_save=True)
    else:
        config = get_config()
        if args.load:

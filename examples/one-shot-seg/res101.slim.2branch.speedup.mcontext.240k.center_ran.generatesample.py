...
from tensorpack.utils.segmentation.segmentation import predict_slider,crop_saliency, visualize_label, predict_scaler
    ...
    def data_prepare(ds):
        if isTrain:
            ...
            for kk in range(k_shots):
                ...
                first_label = cv2.resize(first_label, support_image_size, interpolation=cv2.INTER_NEAREST)
                #first_image_masked = first_image * first_label[:, :, np.newaxis]
                first_image_masks.append([first_image,first_label])
    ...
    for first_image_masks, second_image, second_label  in tqdm(ds.get_data()):
        ...
        for kk in range(k_shot):
            def mypredictor(input_img):
                # input image: 1*H*W*3
                # output : H*W*C
                img,label = first_image_masks[kk]
                mask = img * label[:, :, np.newaxis]
                output = predictor(mask[np.newaxis,:,:,:], input_img)
                return output[0][0]
        ...
        if is_save:
            cv2.imwrite("{}/{}.png".format(result_dir,i),
                        np.concatenate((cv2.resize(first_image_masks[0][0],
                                                   (second_image.shape[1],second_image.shape[0])),
                                        cv2.resize(visualize_label(first_image_masks[0][1],class_num=2),
                                                   (second_image.shape[1], second_image.shape[0])),
                                        cv2.resize(crop_saliency(first_image_masks[0][0], first_image_masks[0][1]),(second_image.shape[1],second_image.shape[0])),
                                        second_image, visualize_label(second_label,class_num=2), visualize_label(prediction_fused,class_num=2)), axis=1))

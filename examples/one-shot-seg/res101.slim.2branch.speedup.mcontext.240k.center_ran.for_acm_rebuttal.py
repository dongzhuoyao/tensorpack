...
def proceed_test(args, is_save = True):
    ...
    for first_image_masks, second_image, second_label  in tqdm(ds.get_data()):
        ...
        for kk in range(k_shot):
            ...
            cv2.imwrite("{}/image{}_logicalor_{}fused.png".format(result_dir, i, kk),visualize_binary(prediction_fused.astype(np.uint8)))

        stat.feed(prediction_fused, second_label)

        if is_save:
            cv2.imwrite("{}/{}.png".format(result_dir,i), np.concatenate((cv2.resize(first_image_masks[0],(second_image.shape[1],second_image.shape[0])),second_image, visualize_binary(second_label), visualize_binary(prediction_fused)), axis=1))

...
def proceed_test(args, is_save = True):
    ...
    pred_config = PredictConfig(
        ...
        output_names=['fusion_feat0','fusion_feat1','fusion_feat2','prob'])
    ...
    for first_image_masks, second_image, second_label  in tqdm(ds.get_data()):
        ...
        for kk in range(k_shot):
            for iii in range(3):
                ...
                prediction = visualize_feat(second_image, mypredictor,top_k=3)
                concated.append(cv2.resize(prediction[0],image_size)) #upsample feature to image size
            ...
            concated.append(visualize_binary_mask(second_image, second_label,color=(0, 0, 255),class_num=2))
            concated.append(visualize_binary_mask(second_image, prediction, color=(0, 255, 0), class_num=2))

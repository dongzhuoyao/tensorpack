...
batch_size = 7
evaluate_every_n_epoch = 1#max_epoch
cityscapes_root = "/data2/dataset/cityscapes"
is_debug = 1
            ...
            def get_logits(image):
                with argscope([Conv2D, MaxPooling, GlobalAvgPooling, BatchNorm], data_format="NHWC"):
                    return resnet_backbone_deeplab(
                        image, num_blocks,
                        preresnet_group if mode == 'preact' else resnet_group_deeplab, block_func, class_num = CLASS_NUM,ASPP = False)
...
def get_data(name, meta_dir, batch_size):
    isTrain = name == 'train'
    ds = dataset.Cityscapes( cityscapes_root, meta_dir, name, shuffle=True)
...
def proceed_validation(args, is_save = False, is_densecrf = False):
    import cv2
    name = "val"
    ds = dataset.Cityscapes(cityscapes_root, args.meta_dir, name)
    ds = BatchData(ds, 1)
    ...
    for image, label in tqdm(ds.get_data()):
        ...
        prediction = predict_scaler(image, mypredictor, scales=[1], classes=CLASS_NUM, tile_size=(IMAGE_H,IMAGE_W), is_densecrf = is_densecrf)
...
class CalculateMIoU(Callback):
    ...
    def _trigger(self):
        ...
        for id,(image, label) in tqdm(enumerate(self.val_ds.get_data())):

...
class Model(ModelDesc):
    ...
    def _get_inputs(self):
        ...
        return [InputDesc(tf.float32, [None, IMAGE_H, IMAGE_W, 3], 'image'),
                ...
                InputDesc(tf.float32, [None, ], 'flag')]
    ...
    def _build_graph(self, inputs):
            ...
            def get_logits(image):
                with argscope([Conv2D, MaxPooling, GlobalAvgPooling, BatchNorm], data_format="NHWC"):
                    return resnet_backbone_deeplab(
                        image, num_blocks,
                        preresnet_group if mode == 'preact' else resnet_group_deeplab, block_func, class_num = CLASS_NUM+1,ASPP = False)

            return get_logits(image)

        image, label, flag_gt = inputs
        image = image - tf.constant([104, 116, 122], dtype='float32')
        label = tf.identity(label, name="label")

        #predict = vgg16(image)
        predict_seg_flag = resnet101(image)

        predict = predict_seg_flag[:,:,:,:-1]
        predict_flag = predict_seg_flag[:,:,:,-1]
        ...
        mse_flag_cost = tf.reduce_mean(tf.square(predict_flag - tf.reshape(flag_gt, [-1, 1, 1])),name="mse")

        costs.append(cost)
        costs.append(mse_flag_cost)
...
def get_data(name, meta_dir, batch_size):
    isTrain = name == 'train'
    from cityscapes_gtav import Cityscapes_GTAV
    ds = Cityscapes_GTAV("/data2/dataset/cityscapes", meta_dir, name, shuffle=True)
...
def get_config(meta_dir, batch_size):
    ...
    return TrainConfig(
        ...
        model=Model(),
        steps_per_epoch=dataset_train.size() * epoch_scale,
        max_epoch = 10 if is_debug else  steps_per_epoch,
        nr_tower = nr_tower
    )
...
class CalculateMIoU(Callback):
    ...
    def _trigger(self):
        ...
        for id, (image, label,_) in tqdm(enumerate(self.val_ds.get_data())):
            if is_debug and id>10:break

            label = np.squeeze(label)
            image = np.squeeze(image)
            prediction = predict_scaler(image, mypredictor, scales=[1], classes=CLASS_NUM, tile_size=(IMAGE_H,IMAGE_W),
                           is_densecrf=False)
            prediction = np.argmax(prediction, axis=2)

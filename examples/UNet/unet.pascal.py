...
def get_data(data_dir,meta_dir,name,batch_size=-1,crop_size=-1):
    ...
    if isTrain:
        shape_aug = [
            imgaug.RandomResize(xrange=(0.7, 1.5), yrange=(0.7, 1.5),aspect_ratio_thres=0.15),
...
def proceed_validation(args):
    ...
    for image, label in ds.get_data():
        ...
        acc_list.append(eval_segm.pixel_accuracy(pre, label))
        macc_list.append(eval_segm.mean_accuracy(pre, label))

    print("evaluation miou = {}".format(sum(miou_list) / len(miou_list)))
    print("evaluation mean pix acc = {}".format(sum(macc_list) / len(macc_list)))
    print("evaluation pix acc = {}".format(sum(acc_list) / len(acc_list)))

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--gpu', type=str, default="1", help='comma separated list of GPU(s) to use.')
    parser.add_argument('--data_dir', default="/data_a/dataset/pascalvoc2012/VOC2012trainval/VOCdevkit/VOC2012", help='dataset dir')

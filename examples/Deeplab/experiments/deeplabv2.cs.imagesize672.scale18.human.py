...
def get_data(name, meta_dir, batch_size):
    ...
    def f(ds):
        ...
        label = ds[1]
        def remap(id):
            if id == 11:# 11 means person in cityscapes
                return 1
            elif id == 255:
                return 255
            else:
...
def proceed_validation(args, is_save = False, is_densecrf = False):
    ...
    logger.info(stat.confusion_matrix_beautify)
    logger.info("IOU: ")
    logger.info(stat.mIoU_beautify)
...
class CalculateMIoU(Callback):
    ...
    def _trigger(self):
        ...
        logger.info("confusion matrix: ")
        logger.info(self.stat.confusion_matrix_beautify)
        logger.info("IOU: ")
        logger.info(self.stat.mIoU_beautify)

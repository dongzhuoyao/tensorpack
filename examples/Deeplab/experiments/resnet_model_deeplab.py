...
def resnet_backbone_deeplab(image, num_blocks, group_func, block_func, class_num, ASPP = False):
    with argscope(Conv2D, nl=tf.identity, use_bias=False,
        ...
        resnet_head = (LinearWrap(image)
                  ...
                  .apply(group_func, 'group2', block_func, 256, num_blocks[2], 2, dilation=1, stride_first=True)
                  .apply(group_func, 'group3', block_func, 512, num_blocks[3], 1, dilation=2, stride_first=False)())
    ...
    output = tf.image.resize_bilinear(output, image.shape[1:3]) # upsample 16X
    return output

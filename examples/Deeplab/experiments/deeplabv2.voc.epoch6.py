...
from seg_utils import  RandomCropWithPadding,RandomResizeSlight

from resnet_model_deeplab import (
    preresnet_group, preresnet_basicblock, preresnet_bottleneck,
    resnet_group_deeplab, resnet_basicblock, resnet_bottleneck_deeplab, se_resnet_bottleneck,
    resnet_backbone_deeplab)


CurrentDataset = dataset.PascalVOC12
CLASS_NUM = CurrentDataset.class_num()
IMAGE_H = 512
IMAGE_W = 512
IGNORE_LABEL = 255
...
def get_data(name, meta_dir, batch_size):
    global args
    isTrain = name == 'train'
    ds = CurrentDataset(args.data_dir, meta_dir, name, shuffle=True)

    if isTrain:
        ds = MapData(ds, RandomResizeSlight)
            ...
            def get_logits(image):
                with argscope([Conv2D, MaxPooling, GlobalAvgPooling, BatchNorm], data_format="NHWC"):
                    return resnet_backbone_deeplab(
                        image, num_blocks,
                        preresnet_group if mode == 'preact' else resnet_group_deeplab, block_func, class_num = CLASS_NUM, ASPP = True)
...
def proceed_validation(args, is_save = True, is_densecrf = False):
    import cv2
    name = "val"
    ds = CurrentDataset(args.meta_dir, name)
    ds = BatchData(ds, 1)

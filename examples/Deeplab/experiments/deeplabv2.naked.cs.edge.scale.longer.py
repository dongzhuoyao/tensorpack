            ...
            def get_logits(image):
                with argscope([Conv2D, MaxPooling, GlobalAvgPooling, BatchNorm], data_format="NHWC"):
                    return resnet_backbone(
                        image, num_blocks,
                        preresnet_group if mode == 'preact' else resnet_group, block_func,  label, edge, class_num = CLASS_NUM, ASPP = False)
...
def get_data(name, meta_dir, batch_size):
    isTrain = name == 'train'
    ds = dataset.CityscapesEdge(meta_dir, name, shuffle=True)
...
def proceed_validation(args, is_save = True, is_densecrf = False):
    import cv2
    ds = dataset.CityscapesEdge(args.meta_dir, "val")
    ds = BatchData(ds, 1)

...
epoch_scale = 15
max_epoch = 10
lr_multi_schedule = [('shk.*/W', 5),('shk.*/b', 10),('aspp.*_conv/W', 5),('aspp.*_conv/b',10)]
batch_size = 12
    ...
    def _trigger(self):
        ...
        for image, label in tqdm(self.val_ds.get_data()):
            ...
            def mypredictor(input_img):
                ...
                return output[0][0]
            prediction = predict_scaler(image,mypredictor, scales=[0.7,0.9,1,1.1,1.3], classes=CLASS_NUM, tile_size=CROP_SIZE,
                           is_densecrf=False)
            prediction = np.argmax(prediction, axis=2)

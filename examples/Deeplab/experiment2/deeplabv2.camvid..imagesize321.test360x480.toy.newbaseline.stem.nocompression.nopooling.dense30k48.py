...
def proceed_validation(args, is_save = False, is_densecrf = False):
    ...
    for image, label in tqdm(ds.get_data()):
        ...
        prediction = predict_scaler(image, mypredictor, scales=[0.5,0.75, 1, 1.25, 1.5], classes=CLASS_NUM, tile_size=[360,480], is_densecrf = is_densecrf)
...
class CalculateMIoU(Callback):
    ...
    def _trigger(self):
        ...
        for image, label in tqdm(self.val_ds.get_data()):
            label = np.squeeze(label)
            image = np.squeeze(image)
            prediction = predict_scaler(image, mypredictor, scales=[0.5,0.75, 1, 1.25, 1.5], classes=CLASS_NUM, tile_size=[360,480],
                           is_densecrf=False)
            prediction = np.argmax(prediction, axis=2)

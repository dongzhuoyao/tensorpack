...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        cost = 0
        from tensorpack.utils.segmentation.loss_function import soft_target_softmax_cross_entropy_with_ignore_label
        for jj,p in enumerate(predict_list):
            current_predict = predict_list[jj]
            cost += soft_target_softmax_cross_entropy_with_ignore_label(logits=current_predict, label=label4d,
                                                       class_num=CLASS_NUM)

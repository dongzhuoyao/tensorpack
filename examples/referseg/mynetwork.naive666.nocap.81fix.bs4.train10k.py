...
from RMI_naive_nocap import LSTM_model
...
VOCAB_SIZE = len(DataLoader(name = "train", max_length=MAX_LENGTH, img_size=IMG_SIZE, train_img_num=train_img_num,test_img_num=test_img_num,quick_eval=quick_eval, regenerate_json=regenerate_json, use_caption=False).word_to_idx.keys())#3224#28645#24022  # careful about the VOCAB SIZE
...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        model = LSTM_model(image, class_num=CLASS_NUM, batch_size=current_batch_size, num_steps= STEP_NUM, mode=mode, vocab_size=VOCAB_SIZE, weights="deeplab")
        predict = model.up
...
def get_data(name, batch_size):
    ...
    ds = DataLoader(name = name, max_length=MAX_LENGTH, img_size=IMG_SIZE,train_img_num=train_img_num,test_img_num=test_img_num,quick_eval=quick_eval, regenerate_json=regenerate_json, use_caption=False)
